...
def plot_spec(
    ...
    if scale_spec == "linear":
        ...
        if plot_main_frequencies:
            [freq_median, freq_min,
                freq_max] = find_most_used_frequencies(f, spec)
        ...
        librosa.display.specshow(
            spec, x_axis="time", y_axis="mel", sr=fs, hop_length=HOP_LEN
        )
...
def plot_audio(fs, data):
    ...
    time = [ti * 1.0 / fs for ti in range(sampled_points)]
    plt.plot(time, data)
    plt.xlabel("t [s]")
    plt.ylabel("P [W]")
    plt.show()
...
def plot_charac_audio(json_file_path, audio_file_path):
    ...
    """
    charac_func = labeling.charac_function_audio(
...
def plot_charac_spec(audio_file_path, window_length, overlap, charac_func_audio):
    ...
    """
    charac_func_spec = labeling.charac_function_spec(
        audio_file_path, window_length, overlap, charac_func_audio
    )
...
def plot_charac_fs(fs, charac_func_fs):
    ...
    t_fs = np.arange(0, duration, step=1.0 / fs)
