...
def charac_function_spec(audio_file_path, window_length, overlap, charac_func_audio):
    ...
    for i in range(size_spec):
        label_local = np.mean(
            charac_func_audio[i * regroup_factor: (i + 1) * regroup_factor]
        )
        if label_local > 0.5:
...
def extract_audio(
    ...
    charac_audio = charac_function_audio(
        json_folder_path + json_file, wav_folder_path + wav_file
    )
    ...
    while (
        positive_extracts < nbre_extracts_pos or negative_extracts < nbre_extracts_neg
    ) and counts < max_counts:
        start_time = round(random.random() *
                           (duration_audio - duration_extract), 2)
        ...
        if labeled_prop > threshold and positive_extracts < nbre_extracts_pos:
            ...
            labels_ext = extract_labels(
                json_folder_path + json_file, start_time, duration_extract
            )
