...
def loadModel():
    ...
    interpreter = tflite.Interpreter(
        os.path.join(SCRIPT_DIRECTORY_PATH,
...
def predict(sample, interpreter, sensitivity, num_predictions):
    ...
    interpreter.set_tensor(
        INPUT_LAYER_INDEX, np.array(sample[0], dtype="float32"))
    interpreter.set_tensor(
        MDATA_INPUT_INDEX, np.array(sample[1], dtype="float32"))
    interpreter.invoke()
...
def analyzeAudioData(
    chunks, lat, lon, week, sensitivity, overlap, interpreter, num_predictions
):
    detections = {}
    start = time.time()
...
def analyze(
    ...
    if len(dataset) == 1:
        try:
            datafile = dataset[0]
            output_metadata["FOLDER"] = (
                os.path.join(
                    ".", os.path.relpath(
                        os.path.split(datafile)[0], os.getcwd())
                )
                + os.path.sep
            )
            output_metadata["IN FILE"] = os.path.split(datafile)[1]
            audioData, clip_length = readAudioData(
                datafile, overlap, sample_rate)
            ...
            if output_path is None:
                output_file = os.path.join(
                    output_metadata["FOLDER"], "result.csv")
        ...
        for datafile in dataset:
            try:
                ...
                output_metadata["CLIP LENGTH"] = clip_length
                df = writeResultsToDf(
                    df, detections, min_conf, output_metadata)
            except KeyboardInterrupt:
                exit("Keyboard interrupt")
            except Exception as exc:
                print("Error in processing file: {}".format(datafile))
                logging.exception(f"Error in processing file: {datafile}")
        if output_path is None:
            output_file = os.path.join(audio_path, "result.csv")
            output_file = os.path.abspath(output_file)
        else:
...
def main():
    ...
    parser.add_argument(
        "--i",
        help="Path to input folder/input file. All the nested folders will also be processed.",
    )
    parser.add_argument(
        "--o",
        default="result.csv",
        help="Absolute path to output folder. By default results are written into the input folder.",
    )
    parser.add_argument(
        "--lat",
        type=float,
    ...
    parser.add_argument(
        "--custom_list",
        default="",
        help="Path to text file containing a list of species. Not used if not provided.",
    )
    parser.add_argument(
        "--filetype",
        default="wav",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Defines maximum number of written predictions in a given 3s segment. Defaults to 10",
    )
